            ...
            <div className="relative">
              <div className="aspect-[3/4] rounded-lg overflow-hidden bg-paper-2 relative">
                ...
                <button
                  ...
                  className="absolute top-4 right-4 w-10 h-10 rounded-full bg-paper/80 backdrop-blur-sm flex items-center justify-center hover:bg-paper transition-colors u-focus"
                ...
                <button
                  onClick={handleShare}
                  className="absolute bottom-4 right-4 w-10 h-10 rounded-full bg-paper/80 backdrop-blur-sm flex items-center justify-center hover:bg-paper transition-colors u-focus"
                ...
                <fieldset className="space-y-2">
                  ...
                  <div className="flex flex-wrap gap-2" role="radiogroup" aria-label="Color options">
                    {product.colors.map((colorOpt) => (
                      ...
                      >
                        {selectedColor === colorOpt.color && (
                          <svg className="w-5 h-5 text-paper" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
              ...
              <fieldset className="space-y-2">
                <legend className="u-label mb-2 flex items-center gap-2">
                  ...
                  {availableSizes.length > 0 && (
                    <span className="text-sm text-muted font-normal">({availableSizes.length} available)</span>
                  )}
                </legend>
                <div className="flex flex-wrap gap-2" role="radiogroup" aria-label="Size options">
                  {availableSizes.map((sizeOpt) => (
                    <button
                      ...
                      className={`relative px-4 py-2.5 min-w-[52px] rounded-md border-2 font-medium text-sm transition-all ${
                        selectedSize === sizeOpt.size
                          ? 'border-ink bg-ink text-paper'
                          : sizeOpt.stock === 0
                          ? 'border-line/50 text-muted line-through cursor-not-allowed'
                    ...
                    >
                      ...
                      {sizeOpt.lowStock && sizeOpt.stock > 0 && (
                        <span className="absolute -top-1 -right-1 w-4 h-4 rounded-full bg-warning text-paper text-[9px] font-bold flex items-center justify-center">
                          {sizeOpt.stock}
                        </span>
                      )}
                    </button>
                  ...
                  {availableSizes.length === 0 && (
                    <span className="text-sm text-muted px-4 py-2">Out of stock</span>
                  )}
                </div>
              </fieldset>
                          ...
                          <tbody>
                            {product.sizeGuide.rows.map((row, i) => (
                              <tr key={i} className="border-b border-line/50">
                                {row.map((cell, j) => (
                                  <td key={j} className="py-2 px-3 text-muted {j === 0 ? 'font-medium' : ''}">{cell}</td>
                                ))}
                              </tr>
                            ))}
                          </tbody>
