...
export default function EditCouponPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const data = await res.json();
      if (!res.ok) {
        setError(data.error?.message || 'Failed to update coupon');
        return;
      }
  ...
  return (
    <div className="space-y-6 max-w-3xl mx-auto">
      <div className="flex items-center gap-4 pb-6 border-b border-zinc-800/80">
        ...
        <div>
          <h1 className="text-2xl font-serif font-bold text-zinc-100 tracking-wide">Edit Coupon</h1>
          <p className="text-xs text-zinc-400 mt-1">Update coupon details and settings.</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6 bg-zinc-900/60 border border-zinc-800/80 rounded-xl p-6 shadow-2xl">
        ...
        <div className="grid sm:grid-cols-2 gap-4">
          <div>
            ...
            <input type="text" value={form.code} onChange={(e) => setForm(f => ({ ...f, code: e.target.value }))} required className="w-full px-4 py-2.5 bg-zinc-800 border border-zinc-700 rounded-lg text-sm text-zinc-100 focus:outline-none focus:ring-1 focus:ring-amber-500/60 font-mono" />
          </div>
          <div>
            <label className="block text-xs font-semibold text-zinc-400 uppercase tracking-wider mb-2">Name</label>
            <input type="text" value={form.name} onChange={(e) => setForm(f => ({ ...f, name: e.target.value }))} className="w-full px-4 py-2.5 bg-zinc-800 border border-zinc-700 rounded-lg text-sm text-zinc-100 focus:outline-none focus:ring-1 focus:ring-amber-500/60" />
          </div>
        </div>
        ...
        <div className="grid sm:grid-cols-2 gap-4">
          ...
          <div>
            ...
            <input type="number" value={form.value} onChange={(e) => setForm(f => ({ ...f, value: e.target.value }))} required min="0" className="w-full px-4 py-2.5 bg-zinc-800 border border-zinc-700 rounded-lg text-sm text-zinc-100 focus:outline-none focus:ring-1 focus:ring-amber-500/60" />
          </div>
        </div>
        ...
        <div className="grid sm:grid-cols-2 gap-4">
          <div>
            ...
            <input type="number" value={form.minCartValue} onChange={(e) => setForm(f => ({ ...f, minCartValue: e.target.value }))} min="0" className="w-full px-4 py-2.5 bg-zinc-800 border border-zinc-700 rounded-lg text-sm text-zinc-100 focus:outline-none focus:ring-1 focus:ring-amber-500/60" />
          </div>
          <div>
            <label className="block text-xs font-semibold text-zinc-400 uppercase tracking-wider mb-2">Max Uses</label>
            <input type="number" value={form.maxUses} onChange={(e) => setForm(f => ({ ...f, maxUses: e.target.value }))} min="1" className="w-full px-4 py-2.5 bg-zinc-800 border border-zinc-700 rounded-lg text-sm text-zinc-100 focus:outline-none focus:ring-1 focus:ring-amber-500/60" />
          </div>
        </div>
